...
Scoped operating manual for Nhi's Social Media Manager agent — one of the three agents named in `../context/priorities.md` (ship by end of year, Dec 2026). This is a sub-OS folder: it adds context on top of the root `../CLAUDE.md`, it doesn't replace it. Nhi's identity, business context, and default voice still come from the root files — read `../CLAUDE.md` first if you haven't.

`AGENTS.md` in this folder mirrors this file for Codex. Update both together.
...
- `../Agent - Content Creator/CLAUDE.md` — the content this agent schedules; also has the Brand Guidelines channel specs (handles, image sizes)
- `../Agent - Research/CLAUDE.md` — audience/performance signal this agent can draw on for timing decisions
